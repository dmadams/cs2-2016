...
void sort(vector<Tuple*> &points, vector<double> &angles, int left, int right)
{
    ...
    int pivot = right;
    double pivot_val = angles[pivot];
    Tuple* pivot_val2 = points[pivot];
    for (int i = left; i < pivot; i ++)
    {
        if (angles[i] >= pivot_val)
        {
            angles[pivot] = angles[i];
            points[pivot] = points[i];
            angles[i] = angles[pivot - 1];
            points[i] = points[pivot - 1];
            angles[pivot - 1] = pivot_val;
            points[pivot - 1] = pivot_val2;
            pivot --;
            i --;
        }
    }
